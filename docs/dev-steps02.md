...
このdev-stepsの開発を終えてのふりかえりです。

- 付箋削除、色変更、共有リンクコピーなどのUI追加ができた
- firebase authenticationを使って認証ログインの機能を簡単に実装できた
- firebase は Google Cloud の機能をラップして提供したようなものであると理解
- firestore Rulesでデータベース読み書きの権限を設定できた
- クラウドアプリの開発としてスタート地点の機能は実装できたと感じた
- プロダクトらしくUIの改善、必要に応じてリファクタリングをしたくなってきたので、次フェーズで対応したい
